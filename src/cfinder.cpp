...
//    imshow("mask without finger", skinMask);
    ...
    Point2f windowPt(float(ms.m10 / ms.m00), float(ms.m01 / ms.m00));
    windowPt += Point2f(float(handBounds.tl().x), float(handBounds.tl().y));

    // project it to screen coordinates
    out.pos = kmath::PointI(
            int(kmath::intervalProject(
                windowPt.x,
                kmath::Interval(
                    float(this->mouseRect.x),
                    float(this->mouseRect.x + this->mouseRect.width)
                ),
                kmath::Interval(
                    float(this->screenRect.x),
                    float(this->screenRect.x + this->screenRect.width - 1)
                )
            )),
            int(kmath::intervalProject(
                windowPt.y,
                kmath::Interval(
                    float(this->mouseRect.y),
                    float(this->mouseRect.y + this->mouseRect.height)
                ),
                kmath::Interval(
                    float(this->screenRect.y),
                    float(this->screenRect.y + this->screenRect.height - 1)
                )
            )));
}
...
void CursorFinder::train(const vector<Mat_<Vec3b>> &negFrames)
{
    ...
    io::print("converting frames and storing face masks...");
    for (auto i : xrange(negFrames.size())) {
        ...
        face::getRects(gray, this->cascade, this->faceRects, min(300.0f / float(negFrames[0].cols), 1.0f));

        if (this->faceRects.empty()) {
            io::print("no face detected, frame", i);
        }
...
//        waitForKeypress();
    ...
    io::print(samples.size(), "samples,", resps.size(), "resps");
        ...
        )
{
    ...
    face::getRects(this->gray, this->cascade, this->faceRects, min(300.0f / float(frame.cols), 1.0f));
    ...
    filter(
            [&](const vector<Point> &ctr) {
                if (contourArea(ctr) < double(this->minCtrProp * float(maxFaceRect.area()))) {
                    return false;
                }
